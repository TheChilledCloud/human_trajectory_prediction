...
for i in range(0, len(testing_tracks_list), 4):
    ...
    df_tracks_ordered = pd.DataFrame()
    df_tracks_ordered['xCenter'] = df_tracks['xCenter']
    df_tracks_ordered['yCenter'] = df_tracks['yCenter']
    df_tracks_ordered['xVelocity'] = df_tracks['xVelocity']
    df_tracks_ordered['yVelocity'] = df_tracks['yVelocity']
    df_tracks_ordered['xAcceleration'] = df_tracks['xAcceleration']
    df_tracks_ordered['yAcceleration'] = df_tracks['yAcceleration']
    df_tracks_ordered['heading'] = df_tracks['heading']
    df_tracks_ordered['trackId'] = df_tracks['trackId']
    df_tracks_ordered['frame'] = df_tracks['frame']    
    df_tracks = df_tracks_ordered
    ...
    df_pedestrians_tracks.columns=['xCenter','yCenter', 'xVelocity', 'yVelocity', 'xAcceleration', 'yAcceleration', 'heading', 'trackId', 'frame']
    ...
    x_test = np.delete(x_test, 8, 2)
    x_test = np.delete(x_test, 7, 2)
    y_test = np.delete(y_test, 8, 1)
    ...
    # adding 5 more columns to the predictions, so total shape is (,7)
    ...
    x_test_copies = np.append(x_test_copies, new_column[:,0:1], axis=1)
